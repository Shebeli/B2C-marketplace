...
from ecom_user_profile.models import Transaction, Wallet
...
def initiate_order_payment(order: Order) -> Order:
    ...
    try:
        ...
        logger.exception(
            f"An unexpected `result` was received from the IPG: {exc}"
        )
        raise serializers.ValidationError(
...
def validate_cart_not_empty(user: EcomUser) -> None:
    ...
    if not user.cart.items.exists():
        raise serializers.ValidationError(
            "The cart doesn't contain any items"
        )
...
def update_order_to_shipped(order: Order, validated_data: dict) -> Order:
    ...
    order's tracking code and status will be also updated.
