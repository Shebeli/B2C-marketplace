...
def handle_payment(self, track_id: int, payment_id: int) -> None:
    ...
    if payment.status not in (Payment.UNPAID, Payment.PAYING):
        logger.info(
            ...
            f"No further call to the IPG will be made for syncing the status of this payment."
        )
        return

    # order is still unpaid, inqurying to make sure that the purchase is completed.
    zibal_client = ZibalIPGClient(
    ...
    try:
        response_data = zibal_client.inquiry_transaction(track_id=track_id)
        if response_data.status == PAID_AND_UNVERIFIED:
            verify_data = zibal_client.verify_transaction(track_id)
...
def cancel_unpaid_order(self, order_id):
    ...
    if order.status == Order.PAYING:
        raise self.retry(countdown=60 * 20)  # retry in 20 minutes
